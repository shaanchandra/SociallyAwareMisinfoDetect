...
def qual_analysis_gossipcop():
    ...
    doc_embeds_file_text = os.path.join('data', 'complete_data', 'gossipcop', 'cached_embeds', 'doc_embeds_roberta_lr_test.pt')
    node2id_file = os.path.join('data', 'complete_data', 'node2id_lr_30_30_gossipcop.json')
    node2id = json.load(open(node2id_file, 'r'))
    test2id_file = os.path.join('data', 'complete_data', 'gossipcop', 'doc2id_encoder.json')
    test2id = json.load(open(test2id_file, 'r'))
    doc_embeds_text = torch.load(doc_embeds_file_text, map_location=torch.device('cpu'))
    
    
    roberta_docs_file = os.path.join('data', 'complete_data', 'gossipcop', 'cached_embeds', 'correct_docs_roberta.json')
    gnn_docs_file = os.path.join('data', 'complete_data', 'gossipcop', 'cached_embeds', 'correct_docs_gcn.json')
    doc2labels_file = os.path.join('data', 'complete_data', 'gossipcop', 'cached_embeds', 'docs_labels_lr_30_30_test.json')
    ...
    fake_user_counts = json.load(open(os.path.join('data', 'complete_data', 'gossipcop', 'tsne', 'user_split_counts_30_30.json')))['fake_users']
    real_user_counts = json.load(open(os.path.join('data', 'complete_data', 'gossipcop', 'tsne', 'user_split_counts_30_30.json')))['real_users']
    doc_user_counts = {}
    for count, doc in enumerate(gnn_not_roberta):
        doc_user_counts[str(doc)] = defaultdict(int)
        file = os.path.join('data', 'complete_data', 'gossipcop', 'complete', '{}.json'.format(doc))
...
def qual_analysis_health():
    ...
    doc_embeds_file_text = os.path.join('data', 'complete_data', 'HealthStory', 'cached_embeds', 'doc_embeds_roberta_21_test.pt')
    node2id_file = os.path.join('data', 'complete_data', 'HealthStory', 'node2id_lr_top10.json')
    node2id = json.load(open(node2id_file, 'r'))
    test2id_file = os.path.join('data', 'complete_data', 'HealthStory', 'doc2id_encoder.json')
    test2id = json.load(open(test2id_file, 'r'))
    doc_embeds_text = torch.load(doc_embeds_file_text, map_location=torch.device('cpu'))
    
    
    roberta_docs_file = os.path.join('data', 'complete_data', 'HealthStory', 'cached_embeds', 'correct_docs_roberta.json')
    gnn_docs_file = os.path.join('data', 'complete_data', 'HealthStory', 'cached_embeds', 'correct_docs_gcn.json')
    doc2labels_file = os.path.join('FakeHealth', 'doc2labels_HealthStory.json')
    ...
    fake_user_counts = json.load(open(os.path.join('FakeHealth', 'user_split_counts_HealthStory.json')))['fake_users']
    real_user_counts = json.load(open(os.path.join('FakeHealth', 'user_split_counts_HealthStory.json')))['real_users']
    doc_user_counts = {}
    for count, doc in enumerate(gnn_not_roberta):
        doc_user_counts[str(doc)] = defaultdict(int)
        file = os.path.join('FakeHealth', 'engagements', 'complete', 'HealthStory', '{}.json'.format(doc))
        users = json.load(open(file, 'r'))['users']
        for user in users:
            if str(user) in fake_user_counts:
                doc_user_counts[str(doc)]['fake_counts']+= fake_user_counts[str(user)]/len(users)
...
def qual_analysis_pheme(fold):
    ...
    doc_embeds_file_text = os.path.join('data', 'complete_data', 'pheme_cv', 'fold_{}'.format(fold), 'doc_embeds_roberta_lr_filtered_21_test.pt')
    node2id_file = os.path.join('data', 'complete_data', 'pheme_cv', 'fold_{}'.format(fold), 'node2id_lr_filtered.json')
    node2id = json.load(open(node2id_file, 'r'))
    test2id_file = os.path.join('data', 'complete_data', 'pheme_cv', 'fold_{}'.format(fold), 'doc2id_encoder_filtered.json')
    user2id_file = os.path.join('data', 'complete_data', 'pheme_cv', 'fold_{}'.format(fold), 'user2id_train_filtered.json')
    test2id = json.load(open(test2id_file, 'r'))
    user2id = json.load(open(user2id_file, 'r'))
    doc_embeds_text = torch.load(doc_embeds_file_text, map_location=torch.device('cpu'))
    
    
    roberta_docs_file = os.path.join('data', 'complete_data', 'pheme_cv', 'fold_{}'.format(fold), 'correct_docs_roberta.json')
    gnn_docs_file = os.path.join('data', 'complete_data', 'pheme_cv', 'fold_{}'.format(fold), 'correct_docs_gat.json')
    doc2labels_file = os.path.join('data', 'complete_data', 'pheme_cv', 'fold_{}'.format(fold), 'doc2labels.json')
    ...
    event = events[fold-1]
    print("Event = ", event)
    fold_docs_file = os.path.join('data', 'complete_data', 'pheme_cv', 'fold_docs_just.json')
    fold_docs = json.load(open(fold_docs_file, 'r'))[str(event)]
    user_file = os.path.join('data', 'complete_data', 'pheme_cv', 'fold_{}'.format(fold), 'user_fold_counts_indiv.json')
    if not os.path.isfile(user_file):
        src_dir = os.path.join('data', 'base_data', 'pheme_cv', str(event))
        true_users, false_users, unverif_users, all_users = defaultdict(int), defaultdict(int), defaultdict(int), defaultdict(int)
        for root, dirs, files in os.walk(src_dir):
            for count, file in enumerate(files):
                doc = file.split('.')[0]
