...
class GCN_PreProcess():
    ...
    def create_aggregate_folder(self):
        ...
        src_dir = os.path.join('./data', 'base_data', 'pheme_cv')
    ...
    def create_doc_user_folds(self):
        ...
        for fold, event in enumerate(events):
                ...
                for file in files:
                    if not file.startswith('annotation'):
                        ...
                        with open(src_file_path, 'r') as j:
                            ...
                            label = self.convert_annotations(annotation, string = False)
                            if label != 2:
            ...
            print("No. of filtered unverif docs = ", filtered)
            print("Len of doc2labels = {}\n".format(len(doc2labels)))
            labels_dict_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'doc2labels_filtered.json')
            ...
            c=0
            event_users, event_docs = set(), set()
            ...
            for root, dirs, files in os.walk(src_dir):
                for file in files:
                    if file.startswith('.') or file.startswith('structure') or file.startswith('annotation'):
                        ...
                        doc = root.split('\\')[-2]
                        if str(doc) in doc2labels and str(doc) not in event_docs:
        ...
        fold_user_file = os.path.join(self.data_dir, 'fold_users_filtered.json')
        print("\nSaving fold users in  :", fold_user_file)
        with open(fold_user_file, 'w+') as j:
            json.dump(all_users, j)
        
        fold_doc_file = os.path.join(self.data_dir, 'fold_docs_filtered.json')
    ...
    def create_dicts(self):
        ...
        for fold, event in enumerate(events):
            ...
            doc_splits_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'doc_splits_filtered.json')
            ...
            for count, user in enumerate(all_users):
                ...
                node_type.append(2)
            user2id_train_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'user2id_train_filtered.json')
            ...
            print("Node_type = ", len(node_type))
            node2id_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'node2id_lr_train_filtered.json')
            print("Saving node2id_lr_train in : ", node2id_file)
            with open(node2id_file, 'w+') as json_file:
                json.dump(node2id, json_file)
                       
            node_type_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'node_type_lr_train_filtered.npy')
            ...
            print("doc2id_train = ", len(doc2id_train))
            doc2id_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'doc2id_train_filtered.json')
            ...
            node2id.update(user2id_train)
            print("node2id size = ", len(node2id))
            node2id_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'node2id_lr_filtered.json')
            print("Saving node2id_lr_train in : ", node2id_file)
            with open(node2id_file, 'w+') as json_file:
                json.dump(node2id, json_file)
    ...
    def create_adj_matrix_tweet(self):
        ...
        for fold, event in enumerate(events):
            ...
            user2id_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'user2id_train_filtered.json')
            doc2id_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'doc2id_train_filtered.json')
            ...
            for root, dirs, files in os.walk(src_dir):
                for count, file in enumerate(files):
                    ...
                    if str(doc_key) in doc2id:
                        ...
                        if str(src_file['source_user']) in user2id:
            ...
            filename = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'adj_matrix_filtered_lr.npz')
            ...
            filename = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'edge_type_filtered_lr.npz')
            ...
            edge_matrix_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'adj_matrix_filtered_lr_edge.npy')
            ...
            edge_matrix_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'edge_type_filtered_lr_edge.npy')
    ...
    def create_feat_matrix(self, binary=True):
        ...
        for fold, event in enumerate(events):
            ...
            for root, dirs, files in os.walk(src_doc_dir):
                for file in files:
                    if file.startswith('.') or file.startswith('structure') or file.startswith('annotation') or root.endswith('reactions'):
                        ...
                        doc = file.split('.')[0]
                        if str(doc) not in test_docs and str(doc) in train_docs:
                            src_file_path = os.path.join(root, file)
                            src_tweet = json.load(open(src_file_path, 'r'))
            ...
            filename = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'feat_matrix_filterednoise_lr.npz')
    ...
    def create_labels(self):
        ...
        for fold, event in enumerate(events):
            ...
            doc_splits_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'doc_splits_filtered.json')
            doc_splits = json.load(open(doc_splits_file, 'r'))
            adj_matrix_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'adj_matrix_filtered_lr.npz')
            adj_matrix = load_npz(adj_matrix_file)
            ...
            filename = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'labels_list_filtered.json')
            temp_dict = {}
            ...
            all_labels_data_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'all_labels_filtered.json')
    ...
    def create_split_masks(self):
        ...
        for fold, event in enumerate(events):
            ...
            user2id_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'user2id_train_filtered.json')
            doc2id_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'doc2id_train_filtered.json')
            split_mask_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'split_mask_filtered.json')
            adj_matrix_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'adj_matrix_filtered_lr.npz')
            doc_splits_file = os.path.join(self.data_dir, 'fold_{}'.format(fold+1), 'doc_splits_filtered.json')
