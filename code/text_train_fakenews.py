...
if __name__ == '__main__':
    ...
    # For FakeNews non-transfomer training
    seeds = [21]
    w_decay = [1e-3]
    hidden_dp = [0.1]
    lrs = [1e-3]

    best_hidden_dp, best_decay, best_lr = 0,0,0
    best_f1, best_macro_f1 = -100, -100
    results = {}   
    ...
    for decay in w_decay:
        ...
        for lr in lrs:
            results[decay][lr] = {}
            config['lr'] = lr
            for hid_drop in hidden_dp:
                results[decay][lr][hid_drop] = {}
                config['dropout'] = hid_drop
                avg_val_recall, avg_test_recall, avg_val_prec, avg_test_prec, avg_val_acc, avg_test_acc = 0,0,0,0, 0, 0
                avg_val_macro_f1, avg_test_macro_f1, avg_val_micro_f1, avg_test_micro_f1, avg_val_f1_weighted, avg_test_f1_weighted = 0,0,0,0,0,0
                avg_val_f1, avg_test_f1 = 0,0
                sig_test = []
                for seed in seeds:
                    config['seed']= seed
                    #print("here seed")
                    print("\n decay = {}, lr = {}, hidden_drop = {}, seed = {}".format(decay, lr, hid_drop, seed))
                    config['writer'] = SummaryWriter(config['vis_path'])
                    
                    doc_encoder = Doc_Encoder_Main(config, train_args)
                    best_val_f1 , best_val_acc, best_val_recall, best_val_precision, test_f1, test_macro_f1, test_accuracy, test_recall, test_precision = doc_encoder.train_main(cache=True)
                    avg_val_f1 += best_val_f1
                    avg_val_prec+= best_val_precision
                    avg_val_recall+= best_val_recall
                    avg_val_acc+= best_val_acc
                    
                    avg_test_macro_f1 += test_macro_f1
                    avg_test_f1 += test_f1
                    sig_test.append(test_f1)
                    avg_test_prec+= test_precision
                    avg_test_recall+= test_recall
                    avg_test_acc+= test_accuracy

                    config['writer'].close()
                results[decay][lr][hid_drop]['macro_f1'] = avg_test_macro_f1 /len(seeds)
                results[decay][lr][hid_drop]['f1'] = avg_test_f1 /len(seeds)
                results[decay][lr][hid_drop]['precision'] = avg_test_prec/len(seeds)
                results[decay][lr][hid_drop]['recall'] = avg_test_recall/len(seeds)
                results[decay][lr][hid_drop]['accuracy'] = avg_test_acc/len(seeds)
                
                if results[decay][lr][hid_drop]['f1'] > best_f1:
                    best_f1 = results[decay][lr][hid_drop]['f1']
                    best_macro_f1 = results[decay][lr][hid_drop]['macro_f1']
                    best_hidden_dp = hid_drop
                    best_decay = decay
                    best_lr = lr
                print("\n\n", results)
                print("Best so far: \nbest_f1 = {},  best_macro_f1 = {},  best hidden_drop = {},  best_lr = {},  best_decay = {}".format(best_f1, best_macro_f1, best_hidden_dp, best_lr, best_decay))
                    
    print("\n\nModel with best f1 score has the configuration:\n\n")
    print("Weight_decay  =  ", best_decay)
    print("LR  =  ", best_lr)
    print("Hiddden dropout  =  ", best_hidden_dp)
    print("\n\n..... with the scores:\n")
    print("Accuracy = ", results[best_decay][best_lr][best_hidden_dp]['accuracy'])
    print("F1 = ", results[best_decay][best_lr][best_hidden_dp]['f1'])
    print("Macro F1 = ", results[best_decay][best_lr][best_hidden_dp]['macro_f1'])
    print("Precision = ", results[best_decay][best_lr][best_hidden_dp]['precision'])
    print("Recall = ", results[best_decay][best_lr][best_hidden_dp]['recall'])
        
        
  
    
        
    # # For FakeNews transfomer training
    # seeds = [3]
    # w_decay = [0.5, 0.1]
    # hidden_dp = [0.1, 0.2, 0.3]
    # attn_dp = [0.1, 0.2, 0.3]
    # lrs = [4e-5, 1e-5]
    # best_hidden_dp, best_attn_dp, best_lr = 0,0,0
    # best_f1, best_macro_f1= -100, -100
    # results = {}
    
    # for decay in w_decay:
    #     results[decay] = {}
    #     train_args['weight_decay'] = decay
    #     # config['weight_decay'] = decay
    #     for lr in lrs:
    #         results[decay][lr] = {}
    ...
    #         for hid_drop in hidden_dp:
    #             results[decay][lr][hid_drop] = {}
    #             config['hidden_dropout_prob'] = hid_drop
    #             for attn_drop in attn_dp:
    #                 results[decay][lr][hid_drop][attn_drop] = {}
    #                 if (hid_drop>0.5 and attn_drop >0.2):
    #                     continue
    #                 config['attention_probs_dropout_prob'] = attn_drop
    #                 avg_val_recall, avg_test_recall, avg_val_prec, avg_test_prec, avg_val_mcc, avg_test_mcc = 0,0,0,0,0,0
    #                 avg_val_f1_pos, avg_val_f1_neg, avg_test_f1_pos, avg_test_f1_neg, avg_val_macro_f1, avg_test_macro_f1, avg_val_micro_f1, avg_test_micro_f1 = 0,0,0,0,0,0,0,0
                    
    #                 for seed in seeds:
    #                     train_args['manual_seed']= seed
    #                     print("\n decay = {}, lr = {}, hidden_drop = {}, attn_drop = {}, seed = {}".format(decay, lr, hid_drop, attn_drop, seed))
    #                     config['model_path'] = os.path.join(config['output_dir_transf'], config['data_name'], config['embed_name'], config['model_name'])
    #                     config['vis_path'] = os.path.join(config['tensorboard_dir_transf'], config['data_name'], config['embed_name'], config['model_name'], str(train_args['learning_rate']), str(config['hidden_dropout_prob']), str(config['attention_probs_dropout_prob']), str(train_args['manual_seed']))
    ...
    #                     val_stats, test_stats, val_mcc, test_mcc = doc_encoder.train_main(cache=False)
    ...
    #                     avg_val_f1_pos+= val_f1_pos
    #                     avg_val_f1_neg += val_f1_neg
    #                     avg_val_macro_f1 += val_macro_f1
    #                     avg_val_micro_f1 +=val_micro_f1
    #                     avg_val_prec+= val_prec
    #                     avg_val_recall+= val_recall
    #                     avg_val_mcc += val_mcc
                        
    #                     avg_test_macro_f1 += test_macro_f1
    #                     avg_test_micro_f1 += test_micro_f1
    #                     avg_test_f1_neg+= test_f1_neg
    #                     avg_test_f1_pos += test_f1_pos
    #                     avg_test_prec+= test_prec
    #                     avg_test_recall+= test_recall
    #                     avg_test_mcc += test_mcc
                        
    #                     config['writer'].close()
    #                 results[decay][lr][hid_drop][attn_drop]['f1_pos'] = avg_test_f1_pos/len(seeds)
    #                 results[decay][lr][hid_drop][attn_drop]['f1_neg'] = avg_test_f1_neg/len(seeds)
    #                 results[decay][lr][hid_drop][attn_drop]['micro_f1'] = avg_test_micro_f1 /len(seeds)
    #                 results[decay][lr][hid_drop][attn_drop]['macro_f1'] = avg_test_macro_f1 /len(seeds)
    #                 results[decay][lr][hid_drop][attn_drop]['precision'] = avg_test_prec/len(seeds)
    #                 results[decay][lr][hid_drop][attn_drop]['recall'] = avg_test_recall/len(seeds)
    #                 results[decay][lr][hid_drop][attn_drop]['mcc'] = avg_test_mcc/len(seeds)
                        
    #                 if results[decay][lr][hid_drop][attn_drop]['f1_pos'] > best_f1:
    #                 # if results[decay][lr][hid_drop][attn_drop]['macro_f1'] > best_macro_f1:
    #                     best_f1 = results[decay][lr][hid_drop][attn_drop]['f1_pos']
    #                     best_macro_f1 = results[decay][lr][hid_drop][attn_drop]['macro_f1']
    #                     best_hidden_dp = hid_drop
    #                     best_attn_dp = attn_drop
    #                     best_lr = lr
    #                     best_decay = decay
    #                 print("\n\n", results)
    #                 print("Best so far: \nbest_f1 = {}, best_macro_f1 = {},  \nbest hidden_drop = {},  best_attn_drop = {},  best_decay = {}, best_lr = {}".format(best_f1, best_macro_f1, best_hidden_dp, best_attn_dp, best_decay, best_lr))
                    
                        
                    
    # print("\n\nModel with best f1 score has the configuration:\n\n")
    # print("Weight Decay = ", best_decay)
    # print("LR  =  ", best_lr)
    # print("Hiddden dropout  =  ", best_hidden_dp)
    # print("Attention dropout  =  ", best_attn_dp)
    # print("\n\n..... with the scores:\n")
    # print("Macro F1 = ", results[best_decay][best_lr][best_hidden_dp][best_attn_dp]['macro_f1'])
    # print("F1 pos = ", results[best_decay][best_lr][best_hidden_dp][best_attn_dp]['f1_pos'])
    # print("Precision = ", results[best_decay][best_lr][best_hidden_dp][best_attn_dp]['precision'])
    # print("Recall = ", results[best_decay][best_lr][best_hidden_dp][best_attn_dp]['recall'])
    # print("Accuracy = ", results[best_decay][best_lr][best_hidden_dp][best_attn_dp]['accuracy'])        
