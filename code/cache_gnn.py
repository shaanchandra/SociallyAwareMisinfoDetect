...
from data import *
from gnn_model import Graph_Net, Relational_GNN
...
class Cache_GNN_Embeds():
    ...
    def predict_and_cache_fakenews(self):
        ...
        #config['device'] = 'cpu'
        #device = 'cpu'
        node2id_file = os.path.join('data', 'complete_data', 'node2id_lr_30_30_{}.json'.format(self.config['data_name']))
        doc2id_file = os.path.join('data', 'complete_data', 'doc2id_lr_train_30_30_{}.json'.format(self.config['data_name']))
        user2id_file = os.path.join('data', 'complete_data', 'user2id_lr_train_30_30_{}.json'.format(self.config['data_name']))
        node2id = json.load(open(node2id_file, 'r'))
